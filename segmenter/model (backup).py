...
def build_model(activation='softmax', loss='categorical_crossentropy', learning_rate=0.001):
    ...
    m = Sequential()
    m.add(Bidirectional(LSTM(64, input_shape=(MAX_LENGTH, len(tokens)))))  # MAX_LENGTH tokens forward, MAX_LENGTH backward
...
def make_samples(sample_indices, step_size=1):
    """ 
    Make semi-redundant length MAX_LENGTH samples from the given indices.
    Returns a tuple (X, Y).
...
if command == 'train':
    indices = load_sample_indices(sys.argv[2])
    ...
    print('Predicting for file of length ', len(indices))
    i = 0
